 ...
 * WebSerial.cpp
 *
 * This file implements the WebSerial class, which provides a web-based serial interface
 ...
 *
 * Usage:
 * - Call begin() to initialize the WebSerial with a server and URL.
...
void WebSerial::begin(AsyncWebServer *server, const char *url)
{
    ...
    _server->on(url, HTTP_GET, [&](AsyncWebServerRequest *request) {
        ...
        AsyncWebServerResponse *response =
            request->beginResponse(200, "text/html", WEBSERIAL_HTML, sizeof(WEBSERIAL_HTML));
...
void WebSerial::onMessage(WSMessageCallback callback)
{
    _onMessageReceived = callback;
}

// onMessage callback setter
void WebSerial::onMessage(WSStringMessageCallback callback)
{
...
void WebSerial::setAuthentication(const String &username, const String &password)
{
    _username                 = username;
    _password                 = password;
